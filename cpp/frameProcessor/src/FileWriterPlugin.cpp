    ...
    timeout_thread_(boost::bind(&FileWriterPlugin::run_close_file_timeout, this))
{
    ...
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_PROCESS_NUMBER, PMDD::UINT_T, PMDA::READ_WRITE, 1, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_PROCESS_RANK, PMDD::UINT_T, PMDA::READ_WRITE, 0, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_PROCESS_BLOCKSIZE, PMDD::UINT_T, PMDA::READ_WRITE, 1, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_PROCESS_BLOCKS_PER_FILE, PMDD::UINT_T, PMDA::READ_WRITE, 0, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_PROCESS_EARLIEST_VERSION, PMDD::BOOL_T, PMDA::READ_WRITE
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_PROCESS_ALIGNMENT_THRESHOLD, PMDD::UINT_T, PMDA::READ_WRITE, 1, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_PROCESS_ALIGNMENT_VALUE, PMDD::UINT_T, PMDA::READ_WRITE, 1, PMD::MAX_UNSET
    );
    ...
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_FILE_NUMBER_START, PMDD::UINT_T, PMDA::READ_WRITE, 0, PMD::MAX_UNSET
    );
    ...
    add_config_param_metadata(
        prefix + FileWriterPlugin::CREATE_ERROR_DURATION, PMDD::UINT_T, PMDA::READ_WRITE, 0, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::WRITE_ERROR_DURATION, PMDD::UINT_T, PMDA::READ_WRITE, 0, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::FLUSH_ERROR_DURATION, PMDD::UINT_T, PMDA::READ_WRITE, 0, PMD::MAX_UNSET
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CLOSE_ERROR_DURATION, PMDD::UINT_T, PMDA::READ_WRITE, 0, PMD::MAX_UNSET
    );
...
void FileWriterPlugin::start_writing()
{
    ...
    if (!writing_) {
        ...
        writing_ = this->current_acquisition_->start_acquisition(
            concurrent_rank_, concurrent_processes_, frames_per_block_, blocks_per_file_, first_file_index_,
            use_file_numbering_, file_postfix_, file_extension_, use_earliest_hdf5_, alignment_threshold_,
            alignment_value_, master_frame_, hdf5_call_durations_
        );
    }
}
...
void FileWriterPlugin::configure(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    try {
        ...
        if (config.has_param(FileWriterPlugin::CONFIG_PROCESS)) {
            OdinData::IpcMessage processConfig(
                config.get_param<const rapidjson::Value&>(FileWriterPlugin::CONFIG_PROCESS)
            );
        ...
        if (config.has_param(FileWriterPlugin::CONFIG_FRAMES)
            ...
            LOG4CXX_INFO(
                logger_, "Expecting " << next_acquisition_->frames_to_write_ << " frames (total " << totalFrames << ")"
            );
        }
...
void FileWriterPlugin::configure_process(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    if (config.has_param(FileWriterPlugin::CONFIG_PROCESS_EARLIEST_VERSION)) {
        ...
        LOG4CXX_DEBUG_LEVEL(
            1, logger_, "Use earliest version of HDF5 library to write file set to " << this->use_earliest_hdf5_
        );
    }
...
void FileWriterPlugin::configure_file(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
        ...
        if (boost::filesystem::exists(p, ec)) {
            ...
            if (boost::filesystem::is_directory(p, ec)) {
                ...
                if (eaccess(file_path.c_str(), W_OK)) {
                    ...
                    this->next_acquisition_->file_path_
                        = config.get_param<std::string>(FileWriterPlugin::CONFIG_FILE_PATH);
                    LOG4CXX_DEBUG_LEVEL(
                        1, logger_, "Next file path changed to " << this->next_acquisition_->file_path_
                    );
                }
            } else {
                std::stringstream ss;
    ...
    if (config.has_param(FileWriterPlugin::CONFIG_FILE_PREFIX)) {
        this->next_acquisition_->configured_filename_
            = config.get_param<std::string>(FileWriterPlugin::CONFIG_FILE_PREFIX);
    ...
    if (config.has_param(FileWriterPlugin::CREATE_ERROR_DURATION)) {
        this->hdf5_error_definition_.create_duration
            = config.get_param<unsigned int>(FileWriterPlugin::CREATE_ERROR_DURATION);
        LOG4CXX_DEBUG_LEVEL(
            1, logger_, "Open error duration changed to " << this->hdf5_error_definition_.create_duration
        );
    }
    if (config.has_param(FileWriterPlugin::WRITE_ERROR_DURATION)) {
        this->hdf5_error_definition_.write_duration
            = config.get_param<unsigned int>(FileWriterPlugin::WRITE_ERROR_DURATION);
        LOG4CXX_DEBUG_LEVEL(
            1, logger_, "Write error duration changed to " << this->hdf5_error_definition_.write_duration
        );
    }
    if (config.has_param(FileWriterPlugin::FLUSH_ERROR_DURATION)) {
        this->hdf5_error_definition_.flush_duration
            = config.get_param<unsigned int>(FileWriterPlugin::FLUSH_ERROR_DURATION);
        LOG4CXX_DEBUG_LEVEL(
            1, logger_, "Flush error duration changed to " << this->hdf5_error_definition_.flush_duration
        );
    }
    if (config.has_param(FileWriterPlugin::CLOSE_ERROR_DURATION)) {
        this->hdf5_error_definition_.close_duration
            = config.get_param<unsigned int>(FileWriterPlugin::CLOSE_ERROR_DURATION);
        LOG4CXX_DEBUG_LEVEL(
            1, logger_, "Close error duration changed to " << this->hdf5_error_definition_.close_duration
        );
    }
}
...
)
{
    ...
    if (config.has_param(FileWriterPlugin::CONFIG_DATASET_COMPRESSION)) {
        dset.compression
            = get_compression_from_string(config.get_param<std::string>(FileWriterPlugin::CONFIG_DATASET_COMPRESSION));
    ...
    if (dset.compression == blosc) {
        LOG4CXX_INFO(
            logger_,
            "Blosc compression settings: compressor=" << dset.blosc_compressor << " level=" << dset.blosc_level
                                                      << " shuffle=" << dset.blosc_shuffle
        );
    }
...
void FileWriterPlugin::create_new_dataset(const std::string& dset_name)
{
    ...
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_DATASET_TYPE, PMDD::STRING_T, PMDA::READ_WRITE, { "h5" }
    );
    ...
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_DATASET_COMPRESSION, PMDD::INT_T, PMDA::READ_WRITE, { 1, 2, 3, 4 }
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_DATASET_BLOSC_COMPRESSOR, PMDD::INT_T, PMDA::READ_WRITE, { 0, 1, 2, 3, 4, 5 }
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_DATASET_BLOSC_LEVEL, PMDD::INT_T, PMDA::READ_WRITE, 1, 9
    );
    add_config_param_metadata(
        prefix + FileWriterPlugin::CONFIG_DATASET_BLOSC_SHUFFLE, PMDD::INT_T, PMDA::READ_WRITE, { 0, 1, 2 }
    );
...
bool FileWriterPlugin::frame_in_acquisition(std::shared_ptr<Frame> frame)
{
    ...
    if (!frame_acquisition_ID.empty()) {
        ...
        if (frame_acquisition_ID == next_acquisition_->acquisition_id_) {
            LOG4CXX_DEBUG_LEVEL(
                1, logger_,
                "Acquisition ID sent in frame matches next acquisition ID. "
            ...
            if (next_acquisition_->file_path_ == current_acquisition_->file_path_) {
                if (!next_acquisition_->configured_filename_.empty()) {
                    if (next_acquisition_->configured_filename_ == current_acquisition_->configured_filename_) {
                        ...
                        LOG4CXX_INFO(
                            logger_,
                            "FrameProcessor will not auto-restart acquisition due to identical filename and path"
                        );
                    }
                }
                if (!next_acquisition_->acquisition_id_.empty()) {
                    if (next_acquisition_->acquisition_id_ == current_acquisition_->acquisition_id_) {
                        ...
                        LOG4CXX_INFO(
                            logger_,
                            "FrameProcessor will not auto-restart acquisition due to identical file path and "
                            "acquisition ID"
                        );
                    }
                }
        ...
        if (timeout_thread_running_) {
            ...
            while (timeout_active_) {
                if (!timeout_condition_.wait_for(lock, std::chrono::milliseconds(timeout_period_), [this] {
                    ...
                    // Event - No need to do anything. The timeout will either wait for another period if active or wait
...
{
    std::vector<std::string> reply = { FileWriterPlugin::START_WRITING, FileWriterPlugin::STOP_WRITING };
    return reply;
}
