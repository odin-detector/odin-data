...
void SharedMemoryController::requestSharedBufferConfig(const bool deferred)
{
    if (deferred) {
        ...
        if (sharedBufferConfigRequestDeferred_ && sharedBufferConfigured_) {
            ...
            OdinData::IpcMessage config_request(
                OdinData::IpcMessage::MsgTypeCmd, OdinData::IpcMessage::MsgValCmdBufferConfigRequest
            );
            ...
            if (bufferID != -1) {
                ...
                if (sbm_) {
                    ...
                    FrameProcessor::FrameMetaData frame_meta(
                        frame_number, "raw", FrameProcessor::raw_64bit, "", std::vector<unsigned long long>()
                    );

                    boost::shared_ptr<SharedBufferFrame> frame;
                    frame = boost::shared_ptr<SharedBufferFrame>(new SharedBufferFrame(
                    ...
                    LOG4CXX_WARN(
                        logger_,
                        "RX thread got notification for buffer " << bufferID
