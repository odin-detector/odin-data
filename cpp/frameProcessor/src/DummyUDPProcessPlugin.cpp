...
void DummyUDPProcessPlugin::process_frame(std::shared_ptr<Frame> frame)
{
    ...
    LOG4CXX_DEBUG(
        logger_,
        "Packets received: " << hdr_ptr->total_packets_received << " expected: " << hdr_ptr->total_packets_expected
    );
    ...
    const void* data_ptr
        = static_cast<const void*>(static_cast<const char*>(frame->get_data_ptr()) + sizeof(DummyUDP::FrameHeader));
...
void DummyUDPProcessPlugin::process_lost_packets(std::shared_ptr<Frame>& frame)
{
    ...
    if (hdr_packets_lost > 0) {
        ...
        LOG4CXX_DEBUG(
            logger_, "Processing " << hdr_packets_lost << " lost packets for frame " << hdr_ptr->frame_number
        );
        ...
        if (packets_lost != hdr_packets_lost) {
            LOG4CXX_WARN(
                logger_,
                "Packet loss mismatch between frame header (" << hdr_packets_lost << ") and packet state ("
                                                              << packets_lost << ")"
            );
        }
