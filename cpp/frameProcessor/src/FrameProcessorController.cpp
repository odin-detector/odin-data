...
const std::string FrameProcessorController::METADATA_TS_KEY = "metadata_ts";
    ...
    try {
        ...
        if (ctrlMsg.get_msg_type() == OdinData::IpcMessage::MsgTypeCmd) {
            ...
            case OdinData::IpcMessage::MsgValCmdConfigure: {
                ...
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Control thread reply message (configure): " << replyMsg.encode());
                break;
            }
            ...
            case OdinData::IpcMessage::MsgValCmdRequestConfiguration: {
                ...
                LOG4CXX_DEBUG_LEVEL(
                    3, logger_, "Control thread reply message (request configuration): " << replyMsg.encode()
                );
                break;
            }
            case OdinData::IpcMessage::MsgValCmdExecute: {
                this->execute(ctrlMsg, replyMsg);
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Control thread reply message (command): " << replyMsg.encode());
                break;
            }
            case OdinData::IpcMessage::MsgValCmdRequestCommands: {
                this->requestCommands(replyMsg);
                LOG4CXX_DEBUG_LEVEL(
                    3, logger_, "Control thread reply message (request commands): " << replyMsg.encode()
                );
                break;
            }
            case OdinData::IpcMessage::MsgValCmdStatus: {
                ...
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Control thread reply message (status): " << replyMsg.encode());
                break;
            }
            case OdinData::IpcMessage::MsgValCmdRequestVersion: {
                this->provideVersion(replyMsg);
                LOG4CXX_DEBUG_LEVEL(
                    3, logger_, "Control thread reply message (request version): " << replyMsg.encode()
                );
                break;
            }
            case OdinData::IpcMessage::MsgValCmdResetStatistics: {
                ...
                LOG4CXX_DEBUG_LEVEL(
                    3, logger_, "Control thread reply message (reset statistics): " << replyMsg.encode()
                );
                break;
            }
...
void FrameProcessorController::configure(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    if (config.has_param(FrameProcessorController::CONFIG_STORE)) {
        OdinData::IpcMessage storeConfig(
            config.get_param<const rapidjson::Value&>(FrameProcessorController::CONFIG_STORE)
        );
        ...
        if (storeConfig.has_param(FrameProcessorController::CONFIG_INDEX)
            ...
            const rapidjson::Value& json_value
                = storeConfig.get_param<const rapidjson::Value&>(FrameProcessorController::CONFIG_VALUE);
            rapidjson::StringBuffer buffer;
    ...
    if (config.has_param(FrameProcessorController::CONFIG_EXECUTE)) {
        OdinData::IpcMessage storeConfig(
            config.get_param<const rapidjson::Value&>(FrameProcessorController::CONFIG_EXECUTE)
        );
        ...
        if (storeConfig.has_param(FrameProcessorController::CONFIG_INDEX)) {
            ...
            if (stored_configs_.count(index) > 0) {
                LOG4CXX_DEBUG_LEVEL(
                    1, logger_, "Applying configuration [" << index << "] => " << stored_configs_[index]
                );
                ...
                if (param_doc.IsArray()) {
                    ...
                    for (rapidjson::SizeType i = 0; i < param_doc.Size(); ++i) {
                        ...
                        OdinData::IpcMessage json_config_msg(
                            param_doc[i], OdinData::IpcMessage::MsgTypeCmd, OdinData::IpcMessage::MsgValCmdConfigure
                        );
                    ...
                    // Create a configuration message
                    OdinData::IpcMessage json_config_msg(
                        param_doc, OdinData::IpcMessage::MsgTypeCmd, OdinData::IpcMessage::MsgValCmdConfigure
                    );
                    // Now submit the config to the controller
                    this->configure(json_config_msg, reply);
                }
    ...
    for (iter = plugins_.begin(); iter != plugins_.end(); ++iter) {
        if (config.has_param(iter->first)) {
            OdinData::IpcMessage subConfig(
                config.get_param<const rapidjson::Value&>(iter->first), config.get_msg_type(), config.get_msg_val()
            );
...
void FrameProcessorController::configurePlugin(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    if (config.has_param(FrameProcessorController::CONFIG_PLUGIN_LOAD)) {
        OdinData::IpcMessage pluginConfig(
            config.get_param<const rapidjson::Value&>(FrameProcessorController::CONFIG_PLUGIN_LOAD)
        );
    ...
    if (config.has_param(FrameProcessorController::CONFIG_PLUGIN_CONNECT)) {
        OdinData::IpcMessage pluginConfig(
            config.get_param<const rapidjson::Value&>(FrameProcessorController::CONFIG_PLUGIN_CONNECT)
        );
    ...
    if (config.has_param(FrameProcessorController::CONFIG_PLUGIN_DISCONNECT)) {
        try {
            ...
            OdinData::IpcMessage pluginConfig(
                config.get_param<const rapidjson::Value&>(FrameProcessorController::CONFIG_PLUGIN_DISCONNECT)
            );
            if (pluginConfig.has_param(FrameProcessorController::CONFIG_PLUGIN_CONNECTION)
                && pluginConfig.has_param(FrameProcessorController::CONFIG_PLUGIN_INDEX)) {
                std::string index = pluginConfig.get_param<std::string>(FrameProcessorController::CONFIG_PLUGIN_INDEX);
                std::string cnxn
                    = pluginConfig.get_param<std::string>(FrameProcessorController::CONFIG_PLUGIN_CONNECTION);
                this->disconnectPlugin(index, cnxn);
            }
        }
        // If the JSON parser fails to parse a dictionary out of the disconnect parameter then it might be a simple
        // string which contains the 'all' keyword.  In this case disconnect all of the plugins.
        catch (OdinData::IpcMessageException& e) {
...
void FrameProcessorController::loadPlugin(const std::string& index, const std::string& name, const std::string& library)
{
    ...
    if (plugins_.count(index) == 0) {
        ...
        boost::shared_ptr<FrameProcessorPlugin> plugin
            = OdinData::ClassLoader<FrameProcessorPlugin>::load_class(name, library);
        if (plugin) {
            ...
            LOG4CXX_ERROR(
                logger_, "Could not load plugin with index [" << index << "], name [" << name << "], check library"
            );
            std::stringstream is;
