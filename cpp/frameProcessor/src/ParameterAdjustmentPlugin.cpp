    ...
    if (parameter_adjustments_.size() != 0) {
        ...
        for (iter = parameter_adjustments_.begin(); iter != parameter_adjustments_.end(); ++iter) {
            try {
                ...
                if (parameter_inputs_.find(iter->first) == parameter_inputs_.end()
                    || parameter_inputs_[iter->first] == "") {
                    ...
                    if (frame->meta_data().has_parameter(input_parameter)) {
                        ...
                        std::stringstream ss;
                        ss << "Unable to get parameter " << input_parameter
                           << " to use as input parameter for adjustment";
    ...
    try {
        ...
        if (config.has_param(PARAMETER_NAME_CONFIG)) {
            ...
            if (parameter_names.size() != 0) {
                for (std::vector<std::string>::iterator iter = parameter_names.begin(); iter != parameter_names.end();
                     ++iter) {
