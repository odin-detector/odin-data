...
)
{
    ...
    for (it = hdf5_errors_.begin(); it != hdf5_errors_.end(); ++it) {
        full_error << "\n  [" << frame << "]: " << it->file_name << ":" << it->line << " in " << it->func_name << ": \""
                   << it->desc << "\"";
        frame++;
    }
 ...
 */
size_t HDF5File::create_file(
    std::string filename,
    size_t file_index,
    bool use_earliest_version,
...
)
{
    ...
    if (use_earliest_version_) {
        ensure_h5_result(
            H5Pset_libver_bounds(fapl, H5F_LIBVER_EARLIEST, H5F_LIBVER_LATEST), "H5Pset_libver_bounds failed"
        );
    } else {
        ensure_h5_result(
            H5Pset_libver_bounds(fapl, H5F_LIBVER_LATEST, H5F_LIBVER_LATEST), "H5Pset_libver_bounds failed"
        );
    }
...
)
{
    ...
    LOG4CXX_TRACE(
        logger_,
        "Writing frame [" << frame.get_frame_number() << "] size ["
                          << get_size_from_enum(frame.get_meta_data().get_data_type()) << "] type ["
                          << frame.get_meta_data().get_data_type() << "] name ["
                          << frame.get_meta_data().get_dataset_name() << "]"
    );
    ...
    LOG4CXX_TRACE(
        logger_,
        "Writing frame offset=" << frame.get_frame_number() << " (" << frame_offset
...
void HDF5File::create_dataset(const DatasetDefinition& definition, int low_index, int high_index)
{
    ...
    if (definition.compression == no_compression) {
        ...
        ensure_h5_result(
            H5Pset_filter(prop, LZ4_FILTER, H5Z_FLAG_MANDATORY, cd_values_length, &cd_values),
            "H5Pset_filter failed to set the LZ4 filter"
        );
        ...
        ensure_h5_result(
            H5Pset_filter(prop, BSLZ4_FILTER, H5Z_FLAG_MANDATORY, cd_values_length, cd_values),
            "H5Pset_filter failed to set the BSLZ4 filter"
        );
        ...
        ensure_h5_result(
            H5Pset_filter(prop, BLOSC_FILTER, H5Z_FLAG_OPTIONAL, cd_values_length, cd_values),
            "H5Pset_filter failed to set the Blosc filter"
        );
    }
    ...
    HDF5Dataset_t dset;
    dset.dataset_id
        = H5Dcreate2(this->hdf5_file_id_, definition.name.c_str(), dtype, dataspace, H5P_DEFAULT, prop, dapl);
    ...
    if (dset.dataset_id < 0) {
        ...
        ensure_h5_result(
            H5Sclose(dataspace), "H5Pclose failed to close the dataspace after failing to create the dataset"
        );
    ...
    if (definition.create_low_high_indexes) {
        ...
        hid_t attr_inl
            = H5Acreate2(dset.dataset_id, "image_nr_low", H5T_STD_I32LE, space_inl, H5P_DEFAULT, H5P_DEFAULT);
        ...
        hid_t attr_inh
            = H5Acreate2(dset.dataset_id, "image_nr_high", H5T_STD_I32LE, space_inh, H5P_DEFAULT, H5P_DEFAULT);
...
void HDF5File::extend_dataset(HDF5Dataset_t& dset, size_t frame_no)
{
    if (frame_no > dset.dataset_dimensions[0]) {
        ...
        ensure_h5_result(
            H5Dset_extent(dset.dataset_id, &dset.dataset_dimensions.front()),
            "H5Dset_extent failed to extend the dataset"
        );
    }
}
