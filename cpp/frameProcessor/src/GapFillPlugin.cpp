...
GapFillPlugin::GapFillPlugin()
{
    ...
    add_config_param_metadata(GapFillPlugin::CONFIG_GRID_SIZE, PMDD::INTARR_T, PMDA::READ_WRITE);
...
bool GapFillPlugin::configuration_valid(boost::shared_ptr<Frame> frame)
{
    ...
    if (frame_dimensions[0] != grid_[0] * chip_[0]) {
        std::stringstream ss;
        ss << "GapFill - Inconsistent frame dimension[0] => " << frame_dimensions[0]
           << " compared with (grid[0] x chip[0]) => " << grid_[0] * chip_[0];
        this->set_error(ss.str());
    } else if (frame_dimensions[1] != grid_[1] * chip_[1]) {
        std::stringstream ss;
        ss << "GapFill - Inconsistent frame dimension[1] => " << frame_dimensions[1]
           << " compared with (grid[1] x chip[1]) => " << grid_[1] * chip_[1];
        this->set_error(ss.str());
    } else if (grid_[1] + 1 > gaps_x_.size()) {
        std::stringstream ss;
        ss << "GapFill - Grid size [1] => " << grid_[1] << " not enough x gap values speficied (should be "
           << grid_[1] + 1 << ")";
        this->set_error(ss.str());
    } else if (grid_[0] + 1 > gaps_y_.size()) {
        std::stringstream ss;
        ss << "GapFill - Grid size [0] => " << grid_[0] << " not enough y gap values speficied (should be "
           << grid_[0] + 1 << ")";
        this->set_error(ss.str());
    } else if (grid_[1] + 1 < gaps_x_.size()) {
        std::stringstream ss;
        ss << "GapFill - Grid size [1] => " << grid_[1] << " too many x gap values speficied (should be "
           << grid_[1] + 1 << ")";
        this->set_error(ss.str());
    } else if (grid_[0] + 1 < gaps_y_.size()) {
        std::stringstream ss;
        ss << "GapFill - Grid size [0] => " << grid_[0] << " too many y gap values speficied (should be "
           << grid_[0] + 1 << ")";
        this->set_error(ss.str());
    } else {
        verified = true;
