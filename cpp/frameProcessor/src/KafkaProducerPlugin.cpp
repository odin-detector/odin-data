...
static void kafka_message_callback(rd_kafka_t* kafka_producer, const rd_kafka_message_t* kafka_message, void* opaque)
{
    ...
    if (kafka_message->err) {
        kafka_producer_plugin->on_message_error(rd_kafka_err2str(kafka_message->err));
    } else {
...
void KafkaProducerPlugin::configure_kafka_servers(std::string servers)
{
    ...
    status = rd_kafka_conf_set(kafka_config, "message.max.bytes", KAFKA_MESSAGE_MAX_BYTES, errBuf, sizeof(errBuf));

    if (status != RD_KAFKA_CONF_OK) {
        LOG4CXX_ERROR(logger_, "Kafka configuration error while setting max message size: " << errBuf);
        return;
    }

    status = rd_kafka_conf_set(kafka_config, "bootstrap.servers", servers.c_str(), errBuf, sizeof(errBuf));

    if (status != RD_KAFKA_CONF_OK) {
        LOG4CXX_ERROR(logger_, "Kafka configuration error while setting botstrap servers" << errBuf);
        return;
    }
    ...
    kafka_producer = rd_kafka_new(RD_KAFKA_PRODUCER, kafka_config, errBuf, sizeof(errBuf));
    if (!kafka_producer) {
...
void* KafkaProducerPlugin::create_message(boost::shared_ptr<Frame> frame, size_t& nbytes)
{
    ...
    if (this->include_parameters_) {
        auto& parameters = frame->get_meta_data().get_parameters();
    ...
    *(reinterpret_cast<uint16_t*>(msg)) = header_size;
    ...
    memcpy(msg + sizeof(uint16_t), string_buffer.GetString(), header_size);
    // copy frame data
    memcpy(msg + sizeof(uint16_t) + header_size, frame->get_data_ptr(), frame->get_data_size());
    nbytes = message_size;
    return msg;
}
...
void KafkaProducerPlugin::enqueue_frame(boost::shared_ptr<Frame> frame)
{
    ...
    if (status) {
        ...
        LOG4CXX_ERROR(logger_, "Error while producing: " << rd_kafka_err2str(rd_kafka_last_error()));
        free(buf);
        frames_lost_++;
    } else {
        frames_sent_++;
