        ...
        if (check_frame_valid(frame)) {
            ...
            if (frames_to_write_ > 0 && frames_written_ == frames_to_write_) {
                if (frames_processed_ >= frames_to_write_) {
                    ...
                    LOG4CXX_INFO(
                        logger_,
                        "Number of frames processed (" << frames_processed_ << ") doesn't match expected ("
                                                       << frames_to_write_ << ")"
                    );
...
bool Acquisition::check_frame_valid(boost::shared_ptr<Frame> frame)
{
    ...
    if (!invalid) {
        ...
        if (frame_compression_type == unknown_compression) {
            ...
            std::stringstream ss;
            ss << "Invalid frame: Frame has compression " << frame_compression_type << ", expected "
               << dataset.compression << " for dataset " << dataset.name
        ...
        if (frame_data_type == raw_unknown) {
            ...
            std::stringstream ss;
            ss << "Invalid frame: Frame has data type " << frame_data_type << ", expected " << dataset.data_type
        ...
        if (frame_dimensions != dataset.frame_dimensions) {
            if (frame_dimensions.size() >= 2 && dataset.frame_dimensions.size() >= 2) {
                ...
                std::stringstream ss;
                ss << "Invalid frame: Frame has dimensions [" << frame_dimensions[0] << "], expected ["
                   << dataset.frame_dimensions[0] << "] for dataset " << dataset.name;
                last_error_ = ss.str();
                LOG4CXX_ERROR(logger_, last_error_);
            } else {
    ...
    if (file_index == current_file_->get_file_index()) {
        ...
        LOG4CXX_TRACE(
            ...
            "Creating new file as frame " << frame_offset << " won't go into file index "
                                          << current_file_->get_file_index() << " as it requires " << file_index
        );
...
size_t Acquisition::adjust_frame_offset(boost::shared_ptr<Frame> frame) const
{
    ...
    LOG4CXX_DEBUG_LEVEL(
        2, logger_, "Raw frame number: " << frame_no << ", Frame offset adjustment: " << frame_offset_adjustment
    );
