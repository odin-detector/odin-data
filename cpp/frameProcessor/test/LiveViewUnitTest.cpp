        ...
        recv_socket_other(ZMQ_SUB)
    {
        ...
        for (int i = 0; i < 10; i++) {
            FrameProcessor::FrameMetaData tmp_frame_meta(
                i, i % 4 ? "data" : "not_data", FrameProcessor::raw_8bit, "test", img_dims,
...
/**
 * Tests to ensure the configuration works, including testing to make sure we can change the address of the live view
 * socket
 */
...
/**
 * tests the basic functionality, passing the plugin a single frame and seeing if it appears the same on the other end
 * of the socket
 */
BOOST_AUTO_TEST_CASE(LiveViewBasicSendTest)
{
    ...
    std::copy(pbuf, pbuf + frame->get_data_size(), buf.begin()); // copy data from array to vector, so it can be
                                                                 // compared
...
/**
 * tests to make sure the downscale factor works. Checks that only those frames with the needed frame number are passed
 * to the live view socket
 */
...
BOOST_AUTO_TEST_CASE(LiveViewDatasetFilterTest)
{
    ...
    BOOST_CHECK_EQUAL(
        dataset_processed_frames.size(), 7
    ); // check to make sure all the frames expected were passed through
}

/**
 * test to make sure the live view plugin can work with data types of more than a byte, and that the data is still
 * preserved when it gets passed through
 */
BOOST_AUTO_TEST_CASE(LiveViewOtherDatatypeTest)
{
    ...
    std::copy(
        pbuf_16, pbuf_16 + frame_16->get_data_size() / 2, buf_16.begin()
    ); // Divide the data size in half, as each byte is only half a point of data
    std::vector<uint16_t> original_16(img_16, img_16 + sizeof img_16 / sizeof img_16[0]);
    BOOST_CHECK_EQUAL_COLLECTIONS(
        buf_16.begin(), buf_16.end(), original_16.begin(), original_16.end()
    ); // test that the data is the same

    doc.Parse(message.c_str());
    BOOST_CHECK_EQUAL(
        doc["dsize"].GetInt(), frame_16->get_data_size()
    ); // test that the plugin got the correct size for the frame
    BOOST_CHECK_EQUAL(
        doc["dtype"].GetString(), DATA_TYPES[frame_16->get_data_type()]
    ); // test that the plugin got the correct data type for the frame
}
...
BOOST_AUTO_TEST_CASE(LiveViewTagFilterTest)
{
    ...
    BOOST_CHECK_EQUAL(
        tagged_processed_frames.size(), 7
    ); // check to make sure all the frames expected were passed through
}
