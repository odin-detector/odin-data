...
#define RAPIDJSON_ASSERT(x)                                                    \
    if (!(x))                                                                  \
        throw OdinData::ParamContainerException("rapidjson assertion thrown");
...
protected:
    ...
    template <typename T> void param_set(T& param, rapidjson::Value& value_obj)
    {
    ...
    template <typename T> void vector_param_set(std::vector<T>& param, rapidjson::Value& value_obj)
    {
        ...
        // parameter vector
        for (rapidjson::Value::ValueIterator itr = value_obj.Begin(); itr != value_obj.End(); ++itr) {
            T val;
            set_value<T>(val, *itr);
            param.push_back(val);
        }
    ...
    template <typename T> void vector_param_get(std::vector<T>& param, rapidjson::Value& value_obj)
    {
