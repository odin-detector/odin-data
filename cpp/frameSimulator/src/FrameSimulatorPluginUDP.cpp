...
void FrameSimulatorPluginUDP::replay_frames()
{
    ...
    for (int f = 0; f < frames_to_replay && 0 < frames_.size(); f++) {
        ...
        LOG4CXX_DEBUG(
            logger_,
            "Frame " + boost::lexical_cast<std::string>(n)
                + " packets: " + boost::lexical_cast<std::string>(num_packets)
        );
        ...
        LOG4CXX_DEBUG(
            logger_,
            "Sent " + boost::lexical_cast<std::string>(frame_bytes_sent) + " bytes in "
                + boost::lexical_cast<std::string>(frame_packets_sent) + " packets for frame "
                + boost::lexical_cast<std::string>(n) + ", dropping "
                + boost::lexical_cast<std::string>(frame_packets_dropped) + " packets ("
                + boost::lexical_cast<std::string>(
                    float(100.0 * frame_packets_dropped) / (frame_packets_dropped + frame_packets_sent)
                )
                + "%)"
        );
    ...
    LOG4CXX_DEBUG(
        logger_,
        "Sent " + boost::lexical_cast<std::string>(total_frames_sent) + " frames with a total of "
            ...
            + boost::lexical_cast<std::string>(
                float(100.0 * total_packets_dropped) / (total_packets_dropped + total_packets_sent)
            )
            + "%)"
    );
}
...
int FrameSimulatorPluginUDP::send_packet(const std::shared_ptr<Packet>& packet, const int& frame) const
{
    ...
    return sendto(
        m_socket, packet->data, packet->size, 0, (struct sockaddr*)(&m_addrs[curr_port_index]),
