...
bool DummyUDPFrameSimulatorPlugin::setup(const po::variables_map& vm)
{
    ...
    LOG4CXX_DEBUG(
        logger_,
        "Using image width: " << image_width_ << " height: " << image_height_ << " packet length: " << packet_len_
    );
...
void DummyUDPFrameSimulatorPlugin::create_frames(const int& num_frames)
{
    ...
    for (int frame = 0; frame < num_frames; frame++) {
        ...
        while (bytes_remaining > 0) {
            ...
            DummyUDPPacketHeader* packet_hdr = reinterpret_cast<DummyUDPPacketHeader*>(packet_data);

            packet_hdr->frame_number = frame;
            packet_hdr->packet_number_flags = (packet_number & DummyUDP::packet_number_mask) | packet_flags;
...
void DummyUDPFrameSimulatorPlugin::extract_frames(const u_char* data, const int& size)
{
    ...
    if (is_SOF) {
        LOG4CXX_DEBUG(
            logger_,
            "SOF marker for frame " << frame_number << " at packet " << packet_number << " total " << total_packets
        );
    ...
    if (is_EOF) {
        LOG4CXX_DEBUG(
            logger_,
            "EOF marker for frame " << frame_number << " at packet " << packet_number << " total " << total_packets
        );
