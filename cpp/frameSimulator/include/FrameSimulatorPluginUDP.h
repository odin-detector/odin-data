 ...
 *
 *  Any frame simulator plugin which reads pcap or creates dummy frames to send over a socket should inherit from this
 * class not FrameSimulatorPlugin 'prepare_packets' (and then 'extract_frames') is called on setup if a pcap file is
 * specified: this takes the content of the pcap file and organises it in frames to store 'create_frames' is called on
 * setup if no pcap file is specified 'replay_frames' is called by simulate: this will replay the created/stored frames
 */
