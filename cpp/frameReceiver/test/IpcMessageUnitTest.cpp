...
#define BOOST_TEST_MODULE "IpcMessageTests"
...
BOOST_AUTO_TEST_CASE(ValidIpcMessageFromString)
{
    ...
    OdinData::IpcMessage validMsgFromString(
        "{\"msg_type\":\"cmd\", "
        "\"msg_val\":\"status\", "
...
BOOST_AUTO_TEST_CASE(InvalidIpcMessageFromString)
{
    ...
    BOOST_CHECK_THROW(
        OdinData::IpcMessage invalidMsgFromString("{\"wibble\" : \"wobble\" \"shouldnt be here\"}"),
...
{
    // Instantiate a  message from a JSON string with valid syntax but an illegal type. Turning off strict validation
    // will prevent an exception at this point but render the message invalid
    OdinData::IpcMessage illegalTypeMsgFromString(
        "{\"msg_type\":\"wrong\", \"msg_val\":\"status\", \"timestamp\" : \"2015-01-27T15:26:01.123456\" }", false
    );
...
{
    // Instantiate a  message from a JSON string with valid syntax but an illegal value. Turning off strict validation
    // will prevent an exception at this point but render the message invalid
    OdinData::IpcMessage illegalValueMsgFromString(
        "{\"msg_type\":\"cmd\", \"msg_val\":\"wrong\", \"timestamp\" : \"2015-01-27T15:26:01.123456\" }", false
    );
...
{
    // Instantiate a  message from a JSON string with valid syntax but an illegal timestamp. Turning off strict
    // validation will prevent an exception at this point but render the message invalid
    OdinData::IpcMessage illegalTimestampMsgFromString(
        "{\"msg_type\":\"cmd\", \"msg_val\":\"status\", \"timestamp\" : \"1 Jan 1970 00:00:00\" }", false
    );
...
{
    // Instantiate a  message from a JSON string with valid syntax but an illegal type - with strict validation on,
    // should throw an IpcMessageException
    BOOST_CHECK_THROW(
        OdinData::IpcMessage illegalTypeMsgFromString(
            "{\"msg_type\":\"wrong\", \"msg_val\":\"status\", \"timestamp\" : \"2015-01-27T15:26:01.123456\" }", true
        ),
        OdinData::IpcMessageException
    );
}
...
{
    // Instantiate a  message from a JSON string with valid syntax but an illegal value - with strict validation on,
    // should throw an IpcMessageException
    BOOST_CHECK_THROW(
        OdinData::IpcMessage illegalValueMsgFromString(
            "{\"msg_type\":\"cmd\", \"msg_val\":\"wrong\", \"timestamp\" : \"2015-01-27T15:26:01.123456\" }", true
        ),
        OdinData::IpcMessageException
    );
}
...
{
    // Instantiate a  message from a JSON string with valid syntax but an illegal timestamp - with strict validation on,
    // should throw an IpcMessageException
    BOOST_CHECK_THROW(
        OdinData::IpcMessage illegalTimestampMsgFromString(
            "{\"msg_type\":\"cmd\", \"msg_val\":\"status\", \"timestamp\" : \"1 Jan 1970 00:00:00\" }", true
        ),
        OdinData::IpcMessageException
    );
}
...
BOOST_AUTO_TEST_CASE(MissingParamsIpcMessageFromStringStrictValidation)
{
    // Instantiate a valid message from a JSON string
    BOOST_CHECK_THROW(
        OdinData::IpcMessage missingParamsMsgFromString(
            "{\"msg_type\":\"cmd\", \"msg_val\":\"status\", \"timestamp\" : \"2015-01-27T15:26:01.123456\" }", true
        ),
        OdinData::IpcMessageException
    );
}
...
BOOST_AUTO_TEST_CASE(TestIpcMessageCreationSpeed)
{
    ...
    BOOST_TEST_MESSAGE(
        "Created and encoded " << numLoops << " IPC messages in " << timeDiff(&start, &end) << " secs, rate " << rate
                               << " Hz"
    );
    ...
    rate = (double)numLoops / deltaT;
    BOOST_TEST_MESSAGE(
        "Created and parsed " << numLoops << " IPC messages from string in " << timeDiff(&start, &end) << " secs, rate "
                              << rate << " Hz"
    );
}
