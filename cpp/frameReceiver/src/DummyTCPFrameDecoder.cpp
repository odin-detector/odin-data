...
void DummyTCPFrameDecoder::request_configuration(const std::string param_prefix, OdinData::IpcMessage& config_reply)
{
...
void* DummyTCPFrameDecoder::get_next_message_buffer(void)
{
    ...
    return static_cast<void*>(static_cast<char*>(current_raw_buffer_) + read_so_far_);
}
...
{
    status_msg.set_param(param_prefix + "class", std::string("DummyTCPFrameDecoder"));
}
