...
void FrameReceiverController::configure(OdinData::IpcMessage& config_msg, OdinData::IpcMessage& config_reply)
{
    ...
    try {
        ...
        configuration_complete_
            = ipc_configured_ & decoder_configured_ & buffer_manager_configured_ & rx_thread_configured_;
...
void FrameReceiverController::configure_ipc_channels(OdinData::IpcMessage& config_msg)
{
    ...
    if (config_msg.has_param(CONFIG_FRAME_READY_ENDPOINT)) {
        std::string frame_ready_endpoint = config_msg.get_param<std::string>(CONFIG_FRAME_READY_ENDPOINT);
    ...
    if (config_msg.has_param(CONFIG_FRAME_RELEASE_ENDPOINT)) {
        std::string frame_release_endpoint = config_msg.get_param<std::string>(CONFIG_FRAME_RELEASE_ENDPOINT);
    ...
    ipc_configured_
        = (ctrl_channel_configured && rx_channel_configured && ready_channel_configured && ready_channel_configured);
}
...
void FrameReceiverController::configure_frame_decoder(OdinData::IpcMessage& config_msg)
{
    ...
    std::string decoder_type = config_msg.get_param<std::string>(CONFIG_DECODER_TYPE, config_.decoder_type_);
    ...
    if (config_msg.has_param(CONFIG_DECODER_CONFIG)) {
        ...
        if (*new_decoder_config != *(config_.decoder_config_)) {
            ...
            LOG4CXX_DEBUG_LEVEL(
                3, logger_, "Built new decoder configuration message: " << config_.decoder_config_->encode()
            );
            need_decoder_reconfig_ = true;
        }
    }
...
void FrameReceiverController::configure_buffer_manager(OdinData::IpcMessage& config_msg)
{
    ...
    std::string shared_buffer_name
        = config_msg.get_param<std::string>(CONFIG_SHARED_BUFFER_NAME, config_.shared_buffer_name_);
    if (shared_buffer_name != config_.shared_buffer_name_) {
        config_.shared_buffer_name_ = shared_buffer_name;
        need_buffer_manager_reconfig_ = true;
    }

    std::size_t max_buffer_mem = config_msg.get_param<std::size_t>(CONFIG_MAX_BUFFER_MEM, config_.max_buffer_mem_);
    ...
    if (need_buffer_manager_reconfig_) {
        ...
        if (frame_decoder_) {
            ...
            LOG4CXX_DEBUG_LEVEL(
                1, logger_,
                "Configured frame buffer manager of total size " << max_buffer_mem << " with " << total_buffers_
                                                                 << " buffers"
            );
...
void FrameReceiverController::configure_rx_thread(OdinData::IpcMessage& config_msg)
{
    ...
    std::string rx_type_str
        = config_msg.get_param<std::string>(CONFIG_RX_TYPE, FrameReceiverConfig::map_rx_type_to_name(config_.rx_type_));
    ...
    std::string rx_address = config_msg.get_param<std::string>(CONFIG_RX_ADDRESS, config_.rx_address_);
    if (rx_address != config_.rx_address_) {
        config_.rx_address_ = rx_address;
        need_rx_thread_reconfig_ = true;
    }

    int rx_recv_buffer_size = config_msg.get_param<int>(CONFIG_RX_RECV_BUFFER_SIZE, config_.rx_recv_buffer_size_);
    ...
    std::string rx_port_list = config_msg.get_param<std::string>(CONFIG_RX_PORTS, current_rx_port_list);
...
void FrameReceiverController::handle_ctrl_channel(void)
{
    ...
    try {
        ...
        case IpcMessage::MsgTypeCmd:
            ...
            case IpcMessage::MsgValCmdConfigure:
                LOG4CXX_DEBUG_LEVEL(
                    3, logger_, "Got control channel configure request from client " << client_identity
                );
            ...
            case IpcMessage::MsgValCmdRequestConfiguration:
                LOG4CXX_DEBUG_LEVEL(
                    3, logger_, "Got control channel read configuration request from client " << client_identity
                );
            ...
            case IpcMessage::MsgValCmdRequestCommands:
                LOG4CXX_DEBUG_LEVEL(
                    3, logger_, "Got control channel read commands request from client " << client_identity
                );
                this->request_commands(ctrl_reply);
                break;

            case IpcMessage::MsgValCmdStatus:
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Got control channel status request from client " << client_identity);
                this->get_status(ctrl_reply);
                break;

            case IpcMessage::MsgValCmdRequestVersion:
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Got control channel version request from client " << client_identity);
                this->get_version(ctrl_reply);
                break;

            case IpcMessage::MsgValCmdResetStatistics:
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Got reset statistics request from client " << client_identity);
                this->reset_statistics(ctrl_reply);
                break;

            case IpcMessage::MsgValCmdShutdown:
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Got shutdown command request from client " << client_identity);
                this->stop(true);
            ...
            case IpcMessage::MsgValCmdExecute:
                ...
                LOG4CXX_DEBUG_LEVEL(3, logger_, "Control thread reply message (command): " << ctrl_reply.encode());
                break;
    ...
    if (!request_ok) {
        LOG4CXX_ERROR(
            logger_, "Error handling control channel request from client " << client_identity << ": " << error_ss.str()
        );
...
void FrameReceiverController::handle_rx_channel(void)
{
    ...
    try {
        ...
        case IpcMessage::MsgTypeCmd:
            ...
            default:
                LOG4CXX_ERROR(logger_, "Got unexpected value on command message from RX thread: " << rx_msg_encoded);
                break;
            }
            break;
        ...
        case IpcMessage::MsgTypeNotify:
            ...
            case IpcMessage::MsgValNotifyFrameReady:
                LOG4CXX_DEBUG_LEVEL(
                    2, logger_,
                    "Got frame ready notification from RX thread"
                    " for frame "
                        << rx_msg.get_param<int>("frame", -1) << " in buffer " << rx_msg.get_param<int>("buffer_id", -1)
                );
            ...
            default:
                LOG4CXX_ERROR(
                    logger_, "Got unexpected value on notification message from RX thread: " << rx_msg_encoded
                );
                break;
            }
            break;
        ...
        default:
            LOG4CXX_ERROR(logger_, "Got unexpected type on message from RX thread: " << rx_msg_encoded);
            break;
        }
    } catch (IpcMessageException& e) {
...
void FrameReceiverController::handle_frame_release_channel(void)
{
    ...
    try {
        ...
        LOG4CXX_DEBUG_LEVEL(4, logger_, "Got message on frame release channel : " << frame_release_encoded);

        if ((frame_release.get_msg_type() == IpcMessage::MsgTypeNotify)
            && (frame_release.get_msg_val() == IpcMessage::MsgValNotifyFrameRelease)) {
            LOG4CXX_DEBUG_LEVEL(
                2, logger_,
                "Got frame release notification from processor"
                " from frame "
                    << frame_release.get_param<int>("frame", -1) << " in buffer "
                    << frame_release.get_param<int>("buffer_id", -1)
            );
...
void FrameReceiverController::get_status(OdinData::IpcMessage& status_reply)
{
    ...
    if (rx_thread_status_) {
        ...
        if (rx_thread_status_->has_param("decoder")) {
            status_reply.set_param("decoder", rx_thread_status_->get_param<const rapidjson::Value&>("decoder"));
        }
    }
...
void FrameReceiverController::execute(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    if (config.has_param("decoder")) {
        OdinData::IpcMessage sub_config(
            config.get_param<const rapidjson::Value&>("decoder"), config.get_msg_type(), config.get_msg_val()
        );
