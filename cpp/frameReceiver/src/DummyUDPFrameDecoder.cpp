...
void DummyUDPFrameDecoder::init(LoggerPtr& logger, OdinData::IpcMessage& config_msg)
{
    ...
    udp_packets_per_frame_
        = config_msg.get_param<unsigned int>(CONFIG_DECODER_UDP_PACKETS_PER_FRAME, udp_packets_per_frame_);

    if (udp_packets_per_frame_ > DummyUDP::max_packets) {
        std::stringstream sstr;
        sstr << "The requested number of UDP packets (" << udp_packets_per_frame_ << ") exceeds the maximum allowed ("
             << DummyUDP::max_packets << ")";
    ...
    if (udp_packet_size_ > DummyUDP::max_packet_size) {
        std::stringstream sstr;
        sstr << "The requested UDP packet size (" << udp_packet_size_ << ") exceeds the maximum allowed ("
             << DummyUDP::max_packet_size << ")";
        throw OdinData::OdinDataException(sstr.str());
    }
...
//!
void DummyUDPFrameDecoder::request_configuration(const std::string param_prefix, OdinData::IpcMessage& config_reply)
{
    ...
    config_reply.set_param(param_prefix + CONFIG_DECODER_UDP_PACKETS_PER_FRAME, udp_packets_per_frame_);
    config_reply.set_param(param_prefix + CONFIG_DECODER_UDP_PACKET_SIZE, udp_packet_size_);
}
        ...
        if (frame_buffer_map_.count(current_frame_seen_) == 0) {
            if (empty_buffer_queue_.empty()) {
                ...
                if (!dropping_frame_data_) {
                    LOG4CXX_DEBUG_LEVEL(
                        2, logger_,
                        "First packet from frame " << current_frame_seen_ << " detected, allocating frame buffer ID "
                    ...
                    LOG4CXX_DEBUG_LEVEL(
                        2, logger_,
                        "Free buffer now available for frame " << current_frame_seen_ << ", allocating frame buffer ID "
                                                               << current_frame_buffer_id_
                    );
                }
            }
...
void* DummyUDPFrameDecoder::get_next_payload_buffer(void) const
{
    ...
    next_receive_location = reinterpret_cast<uint8_t*>(current_frame_buffer_) + get_frame_header_size()
        + (udp_packet_size_ * get_packet_number());
...
void DummyUDPFrameDecoder::monitor_buffers(void)
{
    ...
    while (buffer_map_iter != frame_buffer_map_.end()) {
        ...
        if (elapsed_ms(frame_header->frame_start_time, current_time) > frame_timeout_ms_) {
            ...
            LOG4CXX_DEBUG_LEVEL(
                1, logger_,
                "Frame " << frame_num << " in buffer " << buffer_id << " addr 0x" << std::hex << buffer_addr << std::dec
                         << " timed out with " << frame_header->total_packets_received << " packets received, "
    ...
    LOG4CXX_DEBUG_LEVEL(
        4, logger_,
        get_num_mapped_buffers() << " frame buffers in use, " << get_num_empty_buffers() << " empty buffers available, "
                                 << frames_timedout_ << " incomplete frames timed out, " << packets_lost_
                                 << " packets lost"
    );
}
...
//!
void DummyUDPFrameDecoder::get_status(const std::string param_prefix, OdinData::IpcMessage& status_msg)
{
    status_get_count_++;
...
{
    return reinterpret_cast<DummyUDP::PacketHeader*>(current_packet_header_.get())->packet_number_flags
        & DummyUDP::packet_number_mask;
}
