...
void FrameReceiverRxThread::run_service(void)
{
    ...
    reactor_.register_channel(rx_channel_, std::bind(&FrameReceiverRxThread::handle_rx_channel, this));
    ...
    int tick_timer_id
        = reactor_.register_timer(tick_period_ms_, 0, std::bind(&FrameReceiverRxThread::tick_timer, this));

    // Add the buffer monitor timer to the reactor
    int buffer_monitor_timer_id = reactor_.register_timer(
        frame_decoder_->get_frame_timeout_ms(), 0, std::bind(&FrameReceiverRxThread::buffer_monitor_timer, this)
    );

    // Register the frame release callback with the decoder
    frame_decoder_->register_frame_ready_callback(std::bind(&FrameReceiverRxThread::frame_ready, this, _1, _2));
...
void FrameReceiverRxThread::handle_rx_channel(void)
{
    ...
    try {
        ...
        case IpcMessage::MsgTypeCmd:
            ...
            default:
                LOG4CXX_ERROR(logger_, "Got unexpected value on command message from main thread: " << rx_msg_encoded);
                break;
            }
            break;
        ...
        case IpcMessage::MsgTypeAck:
            ...
            default:
                LOG4CXX_ERROR(
                    logger_, "Got unexpected value on acknowlege message from main thread: " << rx_msg_encoded
                );
                break;
            }
            break;
        ...
        case IpcMessage::MsgTypeNotify:
            ...
            case IpcMessage::MsgValNotifyBufferPrecharge: {
                ...
                if ((start_buffer_id == -1) || (num_buffers == -1)) {
                    ...
                    LOG4CXX_DEBUG_LEVEL(
                        1, logger_,
                        "Precharged " << num_buffers << " empty buffers onto queue, "
            ...
            case IpcMessage::MsgValNotifyFrameRelease: {
                ...
                if (buffer_id != -1) {
                    ...
                    LOG4CXX_DEBUG_LEVEL(
                        3, logger_,
                        "Added empty buffer ID " << buffer_id << " to queue, length is now "
            ...
            default:
                LOG4CXX_ERROR(logger_, "Got unexpected value on notify message from main thread: " << rx_msg_encoded);
                break;
            }
            break;

        default:
            LOG4CXX_ERROR(logger_, "Got unexpected type on message from main thread: " << rx_msg_encoded);
            IpcMessage rx_reply;
