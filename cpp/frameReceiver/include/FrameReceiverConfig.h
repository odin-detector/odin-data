...
public:
    ...
    void as_ipc_message(OdinData::IpcMessage& config_msg)
    {
        ...
        config_msg.set_param<bool>(decoder_config_path + CONFIG_ENABLE_PACKET_LOGGING, enable_packet_logging_);
...
private:
    ...
    std::string
        frame_ready_endpoint_; //!< IPC channel endpoint for transmitting frame ready notifications to other processes
    std::string frame_release_endpoint_; //!< IPC channel endpoint for receiving frame release notifications from other
                                         //!< processes
