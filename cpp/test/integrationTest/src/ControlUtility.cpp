...
void ControlUtility::send_configuration(const std::string& message)
{
    ...
    LOG4CXX_DEBUG(
        logger_,
        "Sending configuration " + process_path_ + " " + message + " (" + boost::lexical_cast<std::string>(process_pid_)
            + ")"
    );
...
void ControlUtility::end()
{
    ...
    if (!kill_message_) {
        LOG4CXX_DEBUG(
            logger_, "Terminating " + process_path_ + "(" + boost::lexical_cast<std::string>(process_pid_) + ")"
        );
    ...
    LOG4CXX_DEBUG(
        logger_, "Requesting termination " + process_path_ + "(" + boost::lexical_cast<std::string>(process_pid_) + ")"
    );
