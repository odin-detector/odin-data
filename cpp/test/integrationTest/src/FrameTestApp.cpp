 ...
 */
int parse_arguments(int argc, char** argv, po::variables_map& vm, LoggerPtr& logger, boost::property_tree::ptree& ptree)
{
    ...
    try {
        ...
        generic.add_options()("json", po::value<std::string>(), "Configuration file")("help", "Print help message");

        po::options_description cmdline_options;
        cmdline_options.add(generic);

        po::parsed_options parsed = po::command_line_parser(argc, argv).options(generic).allow_unregistered().run();
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        BOOST_FOREACH (boost::property_tree::ptree::value_type& vc, pt.get_child("Main")) {
            ...
            ControlUtility* control
                = new ControlUtility(pt, pos_args, command_entry, process, socket_entry, kill_entry, logger);
            utilities.push_back(control);
            if (pt.get<bool>("Main." + process + "." + "process")) {
                processes.push_back(control);
                control->run_process();
                boost::optional<std::string> configuration
                    = pt.get_optional<std::string>("Main." + process + "." + "configure");
                if (configuration)
