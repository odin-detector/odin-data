...
int FrameProcessorApp::parse_arguments(int argc, char** argv)
{
    ...
    try {
        ...
        generic.add_options()("help,h", "Print this help message")("version,v", "Print program version string");

        po::options_description config("Configuration options");
        config.add_options()("debug-level,d", po::value<unsigned int>()->default_value(debug_level),
            "Set the debug level")("log-config,l", po::value<string>(), "Set the log4cxx logging configuration file")(
    ...
    try {
        ...
        if (config_file_ != "") {
            ...
            std::string config_text(
                (std::istreambuf_iterator<char>(config_file_stream)), std::istreambuf_iterator<char>());
        ...
        LOG4CXX_ERROR(
            logger_, "Caught unhandled exception in frameProcessor, application will terminate: " << e.what());
        rc = 1;
    }
