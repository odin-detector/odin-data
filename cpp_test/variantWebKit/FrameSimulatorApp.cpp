...
boost::shared_ptr<FrameSimulator::FrameSimulatorPlugin> get_requested_plugin(const po::variables_map& vm, LoggerPtr& logger)
{
    ...
    boost::filesystem::path libraryPathAndName = boost::filesystem::path(opt_libpath.get_val(vm)) / boost::filesystem::path("lib" + pluginClass + ".so");
    ...
    boost::shared_ptr<FrameSimulator::FrameSimulatorPlugin>& plugin)
{
    ...
    try {
        ...
        generic.add_options()("help", "Print help message")("version", "Print version information")("subargs", po::value<std::vector<std::string>>(), "Detector specific arguments");
        ...
        po::parsed_options parsed = po::command_line_parser(argc, argv).options(generic).positional(pos).allow_unregistered().run();
        ...
        if (opt_detector.is_specified(vm)) {
            ...
            boost::filesystem::path libraryPathAndName = boost::filesystem::path(opt_libpath.get_val(vm)) / boost::filesystem::path("lib" + detector + librarySuffix + ".so");
