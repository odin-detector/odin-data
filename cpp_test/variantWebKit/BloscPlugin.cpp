...
boost::shared_ptr<Frame> BloscPlugin::compress_frame(boost::shared_ptr<Frame> src_frame)
{
    ...
    LOG4CXX_DEBUG_LEVEL(2, logger_, "Blosc compression: frame=" << src_frame->get_frame_number() << " acquisition=\"" << src_frame->get_meta_data().get_acquisition_ID() << "\"" << ss_blosc_settings.str() << " src=" << src_data_ptr << " dest=" << dest_frame->get_image_ptr());
    ...
    LOG4CXX_DEBUG_LEVEL(2, logger_, "Blosc compression complete: frame=" << src_frame->get_frame_number() << " compressed_size=" << compressed_size << " factor=" << factor);
...
void BloscPlugin::update_compression_settings()
{
    ...
    LOG4CXX_DEBUG_LEVEL(1, logger_, "Blosc compression settings: " << " acquisition=\"" << this->current_acquisition_ << "\"" << " compressor=" << p_compressor_name << " threads=" << blosc_get_nthreads() << " clevel=" << this->compression_settings_.compression_level << " doshuffle=" << this->compression_settings_.shuffle << " typesize=" << this->compression_settings_.type_size << " nbytes=" << this->compression_settings_.uncompressed_size);
    ret = blosc_set_compressor(p_compressor_name);
    if (ret < 0) {
        LOG4CXX_ERROR(logger_, "Blosc failed to set compressor: " << " " << this->compression_settings_.blosc_compressor << " " << *p_compressor_name);
...
void BloscPlugin::configure(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    if (config.has_param(BloscPlugin::CONFIG_BLOSC_COMPRESSOR)) {
        ...
        if (blosc_compressor > BLOSC_ZSTD) {
            LOG4CXX_WARN(logger_, "Commanded blosc compressor: " << blosc_compressor << " is invalid. Setting compressor: " << BLOSC_LZ4 << "(" << BLOSC_LZ4_COMPNAME << ")");
