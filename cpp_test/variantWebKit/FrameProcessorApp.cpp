...
int FrameProcessorApp::parse_arguments(int argc, char** argv)
{
    ...
    try {
        ...
        generic.add_options()("help,h",
            "Print this help message")("version,v",
            "Print program version string");

        po::options_description config("Configuration options");
        config.add_options()("debug-level,d", po::value<unsigned int>()->default_value(debug_level), "Set the debug level")("log-config,l", po::value<string>(), "Set the log4cxx logging configuration file")("io-threads", po::value<unsigned int>()->default_value(OdinData::Defaults::default_io_threads), "Set number of IPC channel IO threads")("ctrl", po::value<std::string>()->default_value("tcp://127.0.0.1:5004"), "Set the control endpoint")("config,c", po::value<std::string>()->default_value(""), "File path of inital JSON config for controller");
        ...
        if (vm.count("help")) {
            std::cout << "Usage: frameProcessor [options]" << std::endl
                      << std::endl;
