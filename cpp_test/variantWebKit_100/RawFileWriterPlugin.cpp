...
void RawFileWriterPlugin::configure(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    if (config.has_param(RawFileWriterPlugin::CONFIG_FILE_PATH)) {
        ...
        try {
            ...
            error << "Failed to create directory: " << this->file_path_.c_str()
                  << ", Error: " << e.what();
...
{
    reply.set_param(
        this->get_name() + '/' + RawFileWriterPlugin::CONFIG_FILE_PATH, this->file_path_.string());
...
{
    status.set_param(
        this->get_name() + '/' + RawFileWriterPlugin::STATUS_DROPPED_FRAMES, this->dropped_frames_);
}

int RawFileWriterPlugin::get_version_major() { return ODIN_DATA_VERSION_MAJOR; }

int RawFileWriterPlugin::get_version_minor() { return ODIN_DATA_VERSION_MINOR; }

int RawFileWriterPlugin::get_version_patch() { return ODIN_DATA_VERSION_PATCH; }

std::string RawFileWriterPlugin::get_version_short() { return ODIN_DATA_VERSION_STR_SHORT; }

std::string RawFileWriterPlugin::get_version_long() { return ODIN_DATA_VERSION_STR; }
