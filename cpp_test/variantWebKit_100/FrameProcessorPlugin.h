...
class FrameProcessorPlugin : public IFrameCallback,
                             public OdinData::IVersionedObject,
                             public MetaMessagePublisher {
public:
     ...
     */
    __attribute__((always_inline)) void request_configuration_metadata(
        OdinData::IpcMessage& reply) const
    {
    ...
    void register_callback(
        const std::string& name, boost::shared_ptr<IFrameCallback> cb, bool blocking = false);
...
protected:
    ...
    using enableif_t =
        typename std::enable_if<(is_str_conv<DataType_>::value || is_datatype<DataType_>::value)
            && is_all_vals_vec<Allowed_>::value>::type;
    template <typename DataType_>
    using enableif_str_or_datatype_t = typename std::enable_if<is_str_conv<DataType_>::value
        || is_datatype<DataType_>::value>::type;

    /**
     * Helper functions construct the hash_map's elements in-place
     * \param[in] param: key for the ParamMetadata value-object. it is moved to trigger the
     *move-semantics of the key-element in the pair \param[in] access_mode, min, max: arguments for
     *ParamMetadata's constructor. They are not moved since the constructor of ParamMetadata accepts
     *lvalues as references. \param[in] type:           For this argument, the template function
     *ONLY allows std::string convertible types OR ParamMetadata::Datatype enum values to be passed
     *im! \param[in] allowed_values: For this argument, the template function ONLY allows
     *                                    std::vector<ParamMetadata::allowed_values_t> types to be
     *passed in!
     *
     ...
     *
     ***  // 1st overload is instantiated NOTE that a custom string - ("DOUBLE_ARRAY"), was passed
     *to DataType argument
     *    // allowed_values argument is: {"align1", "align2"}.
     *    add_config_param_metadata("MY_PLUGIN_PARAM_STRING", "DOUBLE_ARRAY", PMDA::READ_ONLY,
     *{"align1", "align2"});
     *
     ***  // 2nd overload is instantiated only 'min' is set to 1, 'max' and 'allowed_values' UNSET
     *state add_config_param_metadata(YourPlugin::CONFIG_PARAM3, PMDD::UINT_T, PMDA::READ_WRITE, 1);
     */
    template <typename DataType, typename Allowed_ = All_val_vec_t,
        typename = enableif_t<DataType, Allowed_>>
    auto add_config_param_metadata(std::string param, DataType type,
        ParamMetadata::AccessMode access_mode, Allowed_&& allowed_values) -> void
    {
        config_metadata_.emplace(std::piecewise_construct, std::forward_as_tuple(std::move(param)),
            std::forward_as_tuple(type, access_mode, allowed_values, ParamMetadata::MIN_UNSET,
                ParamMetadata::MAX_UNSET));
    }

    template <typename DataType, typename = enableif_str_or_datatype_t<DataType>>
    auto add_config_param_metadata(std::string param, DataType type,
        ParamMetadata::AccessMode access_mode, int32_t min = ParamMetadata::MIN_UNSET,
        int32_t max = ParamMetadata::MAX_UNSET) -> void
    {
        std::vector<ParamMetadata::allowed_values_t>
            allowed_vals {}; // This allows us to forward the vector as an lvalue reference to the
                             // constructor
        config_metadata_.emplace(std::piecewise_construct, std::forward_as_tuple(std::move(param)),
    ...
    template <typename DataType, typename Allowed_ = All_val_vec_t,
        typename = enableif_t<DataType, Allowed_>>
    auto add_status_param_metadata(std::string param, DataType type,
        ParamMetadata::AccessMode access_mode, Allowed_&& allowed_values) -> void
    {
        status_metadata_.emplace(std::piecewise_construct, std::forward_as_tuple(std::move(param)),
            std::forward_as_tuple(type, access_mode, allowed_values, ParamMetadata::MIN_UNSET,
                ParamMetadata::MAX_UNSET));
    }
...
private:
     ...
     */
    void add_metadata(
        OdinData::IpcMessage& message, const ParameterMetadataMap_t::value_type& metadata) const
    {
