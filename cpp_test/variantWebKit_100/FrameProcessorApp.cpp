...
FrameProcessorApp::FrameProcessorApp(void)
    : config_file_("")
{
...
FrameProcessorApp::~FrameProcessorApp() { controller_.reset(); }
...
int FrameProcessorApp::parse_arguments(int argc, char** argv)
{
    ...
    try {
        ...
        generic.add_options()("help,h", "Print this help message")(
            "version,v", "Print program version string");

        po::options_description config("Configuration options");
        config.add_options()("debug-level,d", po::value<unsigned int>()->default_value(debug_level),
            "Set the debug level")("log-config,l", po::value<string>(),
            "Set the log4cxx logging configuration file")("io-threads",
            po::value<unsigned int>()->default_value(OdinData::Defaults::default_io_threads),
            "Set number of IPC channel IO threads")("ctrl",
            po::value<std::string>()->default_value("tcp://127.0.0.1:5004"),
            "Set the control endpoint")("config,c", po::value<std::string>()->default_value(""),
            "File path of inital JSON config for controller");
        ...
        if (vm.count("log-config")) {
            ...
            LOG4CXX_DEBUG(
                logger_, "log4cxx config file is set to " << vm["log-config"].as<string>());
        }
        ...
        if (vm.count("io-threads")) {
            ...
            LOG4CXX_DEBUG_LEVEL(1, logger_,
                "Setting number of IO threads to " << vm["io-threads"].as<unsigned int>());
        }

        if (vm.count("ctrl")) {
            ctrl_channel_endpoint_ = vm["ctrl"].as<std::string>();
            LOG4CXX_DEBUG_LEVEL(
                1, logger_, "Setting control channel endpoint to " << ctrl_channel_endpoint_);
        }
...
int FrameProcessorApp::run(void)
{
    ...
    controller_
        = boost::shared_ptr<FrameProcessorController>(new FrameProcessorController(io_threads_));
    ...
    try {
        ...
        if (config_file_ != "") {
            ...
            std::string config_text((std::istreambuf_iterator<char>(config_file_stream)),
                std::istreambuf_iterator<char>());

            // Check for empty JSON and throw an exception.
            if (config_text == "") {
                throw OdinData::OdinDataException(
                    "Incorrect or empty JSON configuration file specified");
            }
            ...
            if (config_json.Parse(config_text.c_str()).HasParseError()) {
                ...
                std::string error_snippet
                    = extract_substr_at_pos(config_text, config_json.GetErrorOffset(), 15);
                msg << "Parsing JSON configuration failed at line "
                    << extract_line_no(config_text, config_json.GetErrorOffset()) << ": "
                    << rapidjson::GetParseError_En(config_json.GetParseError()) << " "
                    << error_snippet;
            ...
            if (config_json.IsArray()) {
                ...
                for (rapidjson::SizeType i = 0; i < config_json.Size(); ++i) {
                    OdinData::IpcMessage config_msg(config_json[i],
                        OdinData::IpcMessage::MsgTypeCmd, OdinData::IpcMessage::MsgValCmdConfigure);
                    configure_controller(config_msg);
                }
            } else {
                // Single level JSON object
                OdinData::IpcMessage json_config_msg(config_json, OdinData::IpcMessage::MsgTypeCmd,
        ...
        LOG4CXX_ERROR(logger_,
            "Caught unhandled exception in frameProcessor, application will terminate: "
                << e.what());
        rc = 1;
    }
...
void FrameProcessorApp::configure_controller(OdinData::IpcMessage& config_msg)
{
    ...
    try {
        ...
        LOG4CXX_ERROR(
            logger_, "Failed to configure controller with message " << config_msg.encode());
    }
}
