...
BOOST_STATIC_ASSERT_MSG(
    BLOSC_VERSION_FORMAT == BLOSC_FORMAT_ODIN_USES, "Error: Wrong version of blosc library");
 ...
 * @param cd_values
 */
void create_cd_values(
    const BloscCompressionSettings& settings, std::vector<unsigned int>& cd_values)
{
    if (cd_values.size() < 7)
        cd_values.resize(7);
    cd_values[0] = 0;
...
boost::shared_ptr<Frame> BloscPlugin::compress_frame(boost::shared_ptr<Frame> src_frame)
{
    ...
    const void* src_data_ptr
        = static_cast<const void*>(static_cast<const char*>(src_frame->get_image_ptr()));
    ...
    compressed_size = blosc_compress(c_settings.compression_level, c_settings.shuffle,
        c_settings.type_size, c_settings.uncompressed_size, src_data_ptr,
    ...
    LOG4CXX_DEBUG_LEVEL(2, logger_,
        "Blosc compression complete: frame=" << src_frame->get_frame_number() << " compressed_size="
                                             << compressed_size << " factor=" << factor);
...
void BloscPlugin::update_compression_settings()
{
    ...
    ret = blosc_compcode_to_compname(
        this->compression_settings_.blosc_compressor, &p_compressor_name);
    ...
    if (ret < 0) {
        LOG4CXX_ERROR(logger_,
            "Blosc failed to set compressor: " << " "
...
void BloscPlugin::configure(OdinData::IpcMessage& config, OdinData::IpcMessage& reply)
{
    ...
    if (config.has_param(BloscPlugin::CONFIG_BLOSC_LEVEL)) {
        // NOTE: we don't catch exceptions here because the get_param basically only throws
        // IpcMessagException
        //       if the parameter isn't found - and we've just checked for that in the previous
        //       line...
        ...
        if (blosc_level < 1) {
            LOG4CXX_WARN(
                logger_, "Commanded blosc level: " << blosc_level << "Capped at lower range: 1");
            blosc_level = 1;
            reply.set_param<std::string>("warning: level", "Capped at lower range: 1");
        } else if (blosc_level > 9) {
            LOG4CXX_WARN(
                logger_, "Commanded blosc level: " << blosc_level << "Capped at upper range: 9");
            blosc_level = 9;
    ...
    if (config.has_param(BloscPlugin::CONFIG_BLOSC_SHUFFLE)) {
        unsigned int blosc_shuffle
            = config.get_param<unsigned int>(BloscPlugin::CONFIG_BLOSC_SHUFFLE);
        // Range checking: 0, 1, 2 are valid values. Anything else result in setting value 0 (no
        // shuffle)
        if (blosc_shuffle > BLOSC_BITSHUFFLE) {
            LOG4CXX_WARN(logger_,
                "Commanded blosc shuffle: " << blosc_shuffle
                                            << " is invalid. Disabling SHUFFLE filter");
            blosc_shuffle = 0;
    ...
    if (config.has_param(BloscPlugin::CONFIG_BLOSC_THREADS)) {
        unsigned int blosc_threads
            = config.get_param<unsigned int>(BloscPlugin::CONFIG_BLOSC_THREADS);
        if (blosc_threads > BLOSC_MAX_THREADS) {
            LOG4CXX_WARN(logger_,
                "Commanded blosc threads: " << blosc_threads
                                            << " is too large. Setting 8 threads.");
            blosc_threads = 8;
    ...
    if (config.has_param(BloscPlugin::CONFIG_BLOSC_COMPRESSOR)) {
        unsigned int blosc_compressor
            = config.get_param<unsigned int>(BloscPlugin::CONFIG_BLOSC_COMPRESSOR);
        if (blosc_compressor > BLOSC_ZSTD) {
            LOG4CXX_WARN(logger_,
                "Commanded blosc compressor: " << blosc_compressor
                                               << " is invalid. Setting compressor: " << BLOSC_LZ4
                                               << "(" << BLOSC_LZ4_COMPNAME << ")");
...
{
    status.set_param(
        this->get_name() + "/compressor", this->compression_settings_.blosc_compressor);
...
int BloscPlugin::get_version_major() { return ODIN_DATA_VERSION_MAJOR; }

int BloscPlugin::get_version_minor() { return ODIN_DATA_VERSION_MINOR; }

int BloscPlugin::get_version_patch() { return ODIN_DATA_VERSION_PATCH; }

std::string BloscPlugin::get_version_short() { return ODIN_DATA_VERSION_STR_SHORT; }

std::string BloscPlugin::get_version_long() { return ODIN_DATA_VERSION_STR; }
