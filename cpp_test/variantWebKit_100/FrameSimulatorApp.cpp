...
static const FrameSimulatorOption<std::string> opt_detector(
    "detector", "Set the detector (Excalibur, Eiger etc.)");
static const FrameSimulatorOption<std::string> opt_libpath(
    "lib-path", "Path to detector plugin library");
static const FrameSimulatorOption<unsigned int> opt_debug_level(
    "debug-level", "Set the debug level");
static const FrameSimulatorOption<std::string> opt_log_config(
    "log-config", "Set the log4cxx logging configuration file");
 ...
 * the 'detector' argument must match the library name prefix i.e.
 * 'lib<detector>FrameSimulatorPlugin.so'
 */
boost::shared_ptr<FrameSimulator::FrameSimulatorPlugin> get_requested_plugin(
    const po::variables_map& vm, LoggerPtr& logger)
{
    ...
    try {
        plugin = OdinData::ClassLoader<FrameSimulator::FrameSimulatorPlugin>::load_class(
    ...
    boost::shared_ptr<FrameSimulator::FrameSimulatorPlugin>& plugin)
{
    ...
    try {
        ...
        generic.add_options()("help", "Print help message")("version", "Print version information")(
        ...
        if (opt_detector.is_specified(vm)) {
            ...
            boost::filesystem::path libraryPathAndName
                = boost::filesystem::path(opt_libpath.get_val(vm))
            ...
            if (vm.count("help")) {
                std::cout << "usage: frameSimulator " + detector
                          << " --lib-path <path-to-detector-plugin> " << std::endl
                          << std::endl;
                std::cout << "  --version            Print version information" << std::endl;
                std::cout << "  --" + opt_debug_level.get_argstring() + "        "
                        + opt_debug_level.get_description()
                          << std::endl;
                std::cout << "  --" + opt_log_config.get_argstring() + "          "
                        + opt_log_config.get_description()
                          << std::endl
                          << std::endl;
