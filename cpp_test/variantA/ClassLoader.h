...
public:
     ...
     */
    ClassLoader(std::string name, maker_t* value) { factory_map()[name] = value; }
     ...
     */
    static bool is_registered(std::string name) { return factory_map().end() != factory_map().find(name); }
};
