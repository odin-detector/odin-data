...
protected:
    ...
    template <typename TYPE> using is_all_vals_vec = std::is_same<typename std::decay<TYPE>::type, All_val_vec_t>;
    template <typename TYPE> using is_datatype = std::is_same<typename std::decay<TYPE>::type, ParamMetadata::Datatype>;
    ...
    using enableif_t = typename std::enable_if<
        (is_str_conv<DataType_>::value || is_datatype<DataType_>::value) && is_all_vals_vec<Allowed_>::value>::type;
    template <typename DataType_>
    using enableif_str_or_datatype_t =
        typename std::enable_if<is_str_conv<DataType_>::value || is_datatype<DataType_>::value>::type;
    ...
    auto add_config_param_metadata(
        std::string               param,
        DataType                  type,
        ParamMetadata::AccessMode access_mode,
        Allowed_&&                allowed_values
    ) -> void
    {
        config_metadata_.emplace(
            std::piecewise_construct, std::forward_as_tuple(std::move(param)),
    ...
    auto add_config_param_metadata(
        ...
        int32_t                   max = ParamMetadata::MAX_UNSET
    ) -> void
    {
        std::vector<ParamMetadata::allowed_values_t> allowed_vals {}; // This allows us to forward the vector as an
                                                                      // lvalue reference to the constructor
        config_metadata_.emplace(
            std::piecewise_construct, std::forward_as_tuple(std::move(param)),
    ...
    template <typename DataType, typename Allowed_ = All_val_vec_t, typename = enableif_t<DataType, Allowed_>>
    auto add_status_param_metadata(
        std::string               param,
        DataType                  type,
        ParamMetadata::AccessMode access_mode,
        Allowed_&&                allowed_values
