...
BOOST_AUTO_TEST_CASE( ReactorTimerTest )
{
    ...
    int max_count = 10;
    int timer_id = reactor.add_timer(10, max_count, boost::bind(&ReactorTestFixture::timer_handler, this));
    reactor.run();
...
BOOST_AUTO_TEST_CASE( ReactorChannelTest )
{
    ...
    reactor.add_channel(recv_channel, boost::bind(&ReactorTestFixture::recv_handler, this));
...
{
    reactor.add_channel(recv_channel, boost::bind(&ReactorTestFixture::recv_handler, this));
    int timer_id = reactor.add_timer(10, 1, boost::bind(&ReactorTestFixture::timed_send_handler, this));
    reactor.run();
